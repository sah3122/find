...
<title>FindPet - 애완동물 분실 신고 서비스</title>
...
<body>
	...
	<script type="text/javascript">
					...
					if(fn_Latlng_check(jsonLossData[i].lat, jsonLossData[i].lng)){
						...
						if(jsonLossData[i].img_std != null && jsonLossData[i].img_std != ""){
						    infowindow = new daum.maps.InfoWindow({
						        content: "<div style='text-align:center; width:148px' width='148px'>"+jsonLossData[i].title+"</div>"+"<img src='/images/find/"+jsonLossData[i].img_std+"' width='148px'/>" // 인포윈도우에 표시할 내용
						    });
						} else {
							infowindow = new daum.maps.InfoWindow({
						        content: "<div style='text-align:center; width:148px' width='148px'>"+jsonLossData[i].title+"</div>" // 인포윈도우에 표시할 내용
						    });
						}
						...
						html += "			<span style='display:block;'><strong>"+jsonLossData[i].title+"</strong></span>";
			...
			if(jsonObject.kind_detail == ""){
			    ...
			    if(jsonObject.kind == "dog"){
			    	...
			    	for(var i = 0; i < dogData.length; i++){
				    	if(dogData[i].kind_num == jsonObject.kind_detail){
				    		$("#detail_kind_detail").val(dogData[i].kind);
				    	}
				    }
			    } else if (jsonObject.kind == "cat"){
			    	catData = ${catData};
			    	for(var i = 0; i < catData.length; i++){
				    	if(catData[i].kind_num == jsonObject.kind_detail){
				    		$("#detail_kind_detail").val(catData[i].kind);
				    	}
				    }
			    } else {
		...
		function fn_open_login(){
			if(auth.signInWithPopup(authProvider)) {
				$("#login_span").text(userInfo.displayName + "님이 로그인 하셨습니다.  |  ");
				$("#logout_text").css("display","");
				$("#login_text").css("display","none");
			} else {
					...
					success : function(data) {
						...
						if (data.result == 'success') {
							...
							for(var i = 0; i < jsonKindData.length ; i++){
								if(kind == "dog"){
									html += "<option value='"+jsonKindData[i].kind_num+"'>"+jsonKindData[i].kind+"</option>";	
								} else {
									html += "<option value='"+jsonKindData[i].kind_num+"'>"+jsonKindData[i].kind+"</option>";
								}
							}
