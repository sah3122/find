...
Example.Modal.prototype = {
    ...
    moveCenter : function() {
        ...
        var top = res_height+$(document).scrollTop() - (this.height / 2); // 화면상의 높이에 스크롤높이를 더한 절대좌표를 top에 저장
        console.log($("#"+this.hash.id).outerHeight()/2);
        console.log(top);
        // css의 top,left 조정
        $("#"+this.hash.id).css("top",top - ($("#"+this.hash.id).outerHeight()/2));
        $("#"+this.hash.id).css("left",left - ($("#"+this.hash.id).outerWidth()/2));
